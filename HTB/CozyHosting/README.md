...
This machine chains common web-app misconfigurations into full compromise, with a strong focus on Spring Boot exposure and operational security mistakes.
...
![alt text](assets/burp2.png)
* `/admin` dashboard as `kanderson`
 ![alt text](assets/anderson.png)
...
![alt text](assets/postgress.png)
* extracted `application.properties` showing DB creds
![alt text](assets/postgrescred.png)
...
If it connects successfully, it drops into a root shell.
 ...
 root shell proof (`id`)
![alt text](assets/privesc.png)

✅ **Root flag**
`/root/root.txt` 
![alt text](assets/done.png)

---

## Findings

- Exposed actuator endpoints leaked valid session data and enabled auth bypass.
- Command injection in administrative functionality led to remote code execution.
- Credentials in application artifacts enabled lateral movement and privileged access.

## Lessons Learned

- Debug and actuator endpoints should never be broadly exposed in production.
- Small validation gaps in “internal tooling” features can become full RCE paths.
- Secrets inside deployment artifacts are routinely leveraged in post-exploitation.
